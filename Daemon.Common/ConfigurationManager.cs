...
namespace Daemon.Common
{
    public class ConfigurationManager
    {
        ...
        /// 默认路径
        /// </summary>
        private static string _defaultPath = Directory.GetCurrentDirectory() + System.IO.Path.DirectorySeparatorChar + "appsettings.json";
        ...
        /// 配置节点关键字
        /// </summary>
        public static string _configSection = "AppSettings";
        ...
        private static void ConfigFinder(string Path)
        {
            ...
            var _configSectionArray = _configSection.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
            if (_configSectionArray.Length == 0)
                _configSectionArray = new string[] { "" };
            if (config_json == null || config_json[_configSectionArray[0]] == null) return;
        ...
        private static void LoadConfiguration()
        {
            ...
            if (_configSectionArray.Length == 0)
                _configSectionArray = new string[] { "" };
            if (config_object[_configSectionArray[0]] != null)
            {
                int i = 0;
                do
                {
                    JToken lastConfigSection = null;
                    for (int j = 0; j < _configSectionArray.Length; j++)
                    {
                        if (j != 0)
                        {
                            if (lastConfigSection != null)
                                lastConfigSection = lastConfigSection[_configSectionArray[j]];
                        }
                        else
                            lastConfigSection = config_object[_configSectionArray[j]];
                    }
                    foreach (JProperty prop in lastConfigSection)
                    {
                        configColltion[prop.Name] = prop.Value.ToString();
                    }
                    i++;
                } while (i < _configSectionArray.Length);
            }
